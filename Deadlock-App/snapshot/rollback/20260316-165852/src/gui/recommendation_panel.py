...
def _normalize_hero_key(value: object) -> str:
...
class RecommendationPanel(QGroupBox):
    ...
    @staticmethod
    def _display_hero_name_only(hero_key: str, hero_name_map: dict[str, str]) -> str:
        key = _normalize_hero_key(hero_key)
        return hero_name_map.get(key, hero_name_map.get(hero_key, hero_key))
    ...
    def _populate_bans(self) -> None:
        ...
        for _, ban in meta.sort_values("meta_score", ascending=False).iterrows():
            hero = _normalize_hero_key(ban.get("hero", ""))
            ...
            self._ban_table.setItem(row, 0, QTableWidgetItem(self._display_hero_name_only(hero, hero_name_map)))
