...
def sync_roles_from_metadata() -> dict[str, list[str]]:
    ...
    Roles not in _ROLE_BUCKETS are mapped to 'flex'.  Returns the generated
    mapping and writes it to config/roles.json.
    """
    metadata: dict = load_json(CONFIG_DIR / "hero_metadata.json", default={})
    roles: dict[str, list[str]] = {r: [] for r in _ROLE_BUCKETS}
    for slug, info in metadata.items():
        role = str(info.get("role") or "flex").lower()
        bucket = role if role in roles else "flex"
        roles[bucket].append(slug)
    for bucket in roles:
        roles[bucket].sort()
