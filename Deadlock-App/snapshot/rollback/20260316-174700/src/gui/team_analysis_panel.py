...
_ROLE_KEYWORDS: dict[str, str] = {
    ...
    "assassin": "Assassin",
    "flex": "Flex",
    # anything not recognised (e.g. "unknown") falls back to Flex in _TeamBox.update()
}
...
class _TeamBox(QGroupBox):
    ...
    def update(self, heroes: list[str], metadata: dict) -> None:
        ...
        for hero in heroes:
            ...
            # Unknown/unmapped roles fall back to Flex so heroes are never dropped
            mapped = _ROLE_KEYWORDS.get(role_raw, "Flex")
            counts[mapped] += 1
