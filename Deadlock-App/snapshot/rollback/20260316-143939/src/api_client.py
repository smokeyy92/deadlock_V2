...
class DeadlockAPIClient:
    ...
    def get_matches(self, limit: int = MATCH_PULL_LIMIT) -> list[dict[str, Any]]:
        ...
        for row in rows:
            ...
            # Bulk metadata path already applies min_average_badge server-side.
            # For fallback rows without that filter, enforce it client-side using team average.
            if isinstance(badge0, (int, float)) and isinstance(badge1, (int, float)):
                avg_badge = (float(badge0) + float(badge1)) / 2.0
                if avg_badge < MIN_BADGE_LEVEL:
                    continue
